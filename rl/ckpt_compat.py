...
* 17 scalars -> 20 scalars (tier column removal + power-bar layout remap).
...
Optimizer moments are stripped whenever a migration is materialized.
"""
...
def scalpel_policy_state_dict_to_candidate_maskable_policy(
    ...
    *,
    ...
    leading sub-block matches. **action_net** (35000ΓåÆ4096) stays at template init.
    ...
    if isinstance(fc0_w, torch.Tensor) and isinstance(dst0, torch.Tensor) and fc0_w.dim() == 2:
        ...
        if in_f == int(FUSED_CHANNELS) and int(dst0.shape[1]) == int(FUSED_CHANNELS) + 64:
            ...
            copied.append(f"{fe}fc.0 legacy 144ΓåÆ208 board slice (rows={rows})")
        elif (
    ...
    if isinstance(fc2_w, torch.Tensor) and isinstance(dst2, torch.Tensor) and fc2_w.dim() == 2:
        ...
        copied.append(f"{fe}fc.2 overlaid top-left {r}├ù{c}")
    ...
    if (
        ...
        copied.append(f"value_net overlaid leading {r}├ù{c}")
    ...
    try:
        ...
        if is_temp:
            try:
                ...
                pass
def checkpoint_needs_spatial_stem_patch(ckpt_path: str | Path) -> bool:
    ...
    if isinstance(sw, torch.Tensor) and sw.shape[1] != N_SCALARS:
        return True
    return False
def materialize_sb3_zip_with_spatial_compat(ckpt_path: str | Path) -> tuple[Path, bool]:
    """
...
def _register_numpy2_unpickle_shim() -> None:
    ...
    # ``numpy._core.numeric`` is not importable ΓÇö do not use ``hasattr(np, "_core")``.
    try:
