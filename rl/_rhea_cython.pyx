...
def simulate_genome_cython(
    ...
    for gene_idx in range(len(genome)):
        ...
        # Re-validate: the candidate must still be applicable in the current state.
        # The genome index was generated for the ORIGINAL state; after previous
        # actions mutate `sim`, the candidate may no longer be applicable.
        # Instead of skipping (wasting a genome slot), pick the
        # top-ranked candidate for the CURRENT state.
        if cand not in legal:
            # Candidate is no longer legal/applicable — use best available
            if ranked:
                cand = ranked[0]  # Best candidate for current state
            else:
