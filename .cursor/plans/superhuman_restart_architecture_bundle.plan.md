...
todos:
  ...
  - id: phi-default-flip-and-learner-frame
    content: Flip default `AWBW_REWARD_SHAPING` to `phi` per MASTERPLAN §11.4 AND rewrite `_compute_phi` in [rl/env.py](c:\Users\phili\AWBW\rl\env.py) to compute Φ in the **learner frame** (me − enemy) instead of the hardcoded P0 frame (`p0_val − p1_val`, `cap_p0 − cap_p1`). Same for the legacy `level` reward path while it still exists. This is a shape-locked return-distribution change that V(s) will regress against, so it must ride the same restart. Wave-1 ego-centric refactor map (`docs/restart_arch/ego_centric_refactor_map.md` §7) flagged this as the dependency for any seat-balanced training. Pre-PPO, run [tools/phi_smoke.py](c:\Users\phili\AWBW\tools\phi_smoke.py) on Misery T3 mirror per §11.2 and record numbers under the new contract.
    status: completed
  - id: seat-balanced-actor
    content: Per MASTERPLAN §9 doctrine, alternate which engine seat the **learner** controls across [rl/self_play.py](c:\Users\phili\AWBW\rl\self_play.py) workers (half learner-as-P0 vs pool-as-P1, half learner-as-P1 vs pool-as-P0). Do **not** run policy gradients on actions from the frozen pool opponent — those are off-policy under PPO's clipped surrogate. Critic may train on both seats with sign-flipped returns under the zero-sum assumption; gate that behind a flag so we can A/B it.
    status: completed
  - id: pfsp-opponent-sampler
    content: Add prioritized fictitious self-play sampling on top of the existing `_CheckpointOpponent` rotation in [rl/self_play.py](c:\Users\phili\AWBW\rl\self_play.py). Weight historical checkpoints by `(1 − win_rate_against)^p` so the trainee gets the opponents it currently loses to. Default `p=2`; expose as `--pfsp-power`. This addresses the "counters all-in vs counters turtle" framing without adopting a full multi-population league yet.
    status: completed
  - id: async-vector-env
...
critique of [rl/network.py](c:\Users\phili\AWBW\rl\network.py): the current
...
| Topic | Where |
|------|--------|
| Shipped policy/value net (restart stack) | [rl/network.py](c:\Users\phili\AWBW\rl\network.py) |
| Current encoder (channels and scalars) | [rl/encoder.py](c:\Users\phili\AWBW\rl\encoder.py) |
| Action space and mask construction | [rl/env.py](c:\Users\phili\AWBW\rl\env.py) |
| Self-play loop, opponent rotation, checkpoint pool | [rl/self_play.py](c:\Users\phili\AWBW\rl\self_play.py) |
| MaskablePPO trainer entry | [rl/ppo.py](c:\Users\phili\AWBW\rl\ppo.py), [train.py](c:\Users\phili\AWBW\train.py) |
| Reward shaping (Φ implementation) | [rl/env.py](c:\Users\phili\AWBW\rl\env.py), [tools/phi_smoke.py](c:\Users\phili\AWBW\tools\phi_smoke.py) |
| Phase / gate strategy | [MASTERPLAN.md](c:\Users\phili\AWBW\MASTERPLAN.md) §1, §3, §9, §11 |
| Curriculum execution plan | [.cursor/plans/phase1-foundation-validation.plan.md](c:\Users\phili\AWBW\.cursor\plans\phase1-foundation-validation.plan.md) |
| Reward-shaping plan (Φ origin) | [.cursor/plans/rl_capture-combat_recalibration_4ebf9d22.plan.md](c:\Users\phili\AWBW\.cursor\plans\rl_capture-combat_recalibration_4ebf9d22.plan.md) |
...
## Architecture sketch (shipped; detail + param math in [docs/restart_arch/compute_budget.md](c:\Users\phili\AWBW\docs\restart_arch\compute_budget.md))
...
existing
[.cursor/plans/phase1-foundation-validation.plan.md](c:\Users\phili\AWBW\.cursor\plans\phase1-foundation-validation.plan.md);
...
```text
c:\Users\phili\AWBW\.cursor\plans\superhuman_restart_architecture_bundle.plan.md
```
